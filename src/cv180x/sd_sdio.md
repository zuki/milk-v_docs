...
1. CMDラインが使用中でないことを確認するために `PRESENT[CMD_INHIBIT]` レジスタ
   ビットが0であることを確認します
2. CMDラインがアイドルの場合、ビジーコマンドであるか否を確認します。ビジー
...
4. レジスタビット `PRESENT[DAT_INHIBIT]`が0であるか否かをチェックしてDATAラインが
   使用中であるか否かを確認します。使用中であれば送信が終了まで待機してから5項を
   実行します
...
### 12.4.2.7 同期DMAによるデータ転送モード

同期DMAによるデータ転送モードの手順を[図12-38](images/fig_12-38.png)に示します。
...
### 12.4.2.8 非同期DMAによるデータ転送モード

非同期DMAによるデータ転送モードの手順を[図12-39](images/fig_12-39.png)に示します。
