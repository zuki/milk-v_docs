...
`RTC_NOPOR_INFO0-3`) に保存することができます。2つ目のカウンタはカウントを続け、
...
32KHzクロックと第2パルス周期はソフトウェアプロセスで校正することも、ハードウェア
モジュールを有効にして定期的な自動校正を実行することもできます。
...
リセットすることはできません。最初の電源投入時のPORを除き、例外が発生した際にはRSTN
...
チップの初回電源投入後、システムはRTCを初期化する必要があります。32KHzクロックと第2
時間周期をフラットに較正する必要があります。較正回路は25MHzの水晶クロックを使用して
...
表3.11 `RTC_CTRL`レジスタの概要（ベースアドレス: `0x0300_4000`）
