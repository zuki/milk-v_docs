...
## 1. arm64版xv6のemmcをそのまま使用

### 1.1 QEMU、実機ともにエラー

- QEMUのvirtはSDカードをサポートしていないのでSDのテストはできない
- QEMUの使用は諦める
- xv6実行時に以下のエラー発生
...
- これはSD0領域をpageマッピングしていないせいだった
- SD0のメモリ範囲: `0x04310000`から64KB をmapping定義
- SDカードのセクタ1, 2のダンプ出力は以下の通り
- 読み込み動作自体はエラーなしで実行されるが値はでたらめ
...
- セクタ1,2の1024バイトを出力するとセクタ1の内容が上と異なる
...
- コマンド送信をするところまで進んだがタイムアウトエラーで送信は失敗する
...
## 2. milk-v版のU-Bootからsdhci.cを導入する

### 2.1 `sdhci_init()` の初期化まで実装
...
### 2.3 このタイムアウトは`get_timer()`関数の実装バグだった
...
- sdhci版はread/writeの実装を検討が必要なため、sdhci版で得た初期化の知見をemmcに適用することにした
- 依然としてタイムアウトでコマンドが送信できない
...
- 初期化関数を再調査したところ`emmc_setup_io()`の結果がおかしいことを発見
...
[0]emmc_setup_io:  PWR_EN: 136, CLK: 136, CMD: 4, D0: 4, D1: 4, D2: 4, D3: 4, CD: 4
...
[0]emmc_dump_clk_ctl:  int_clk_en    : 0
[0]emmc_dump_clk_ctl:  int_clk_stable: 0
[0]emmc_dump_clk_ctl:  sd_clk_em     : 1
[0]emmc_dump_clk_ctl:  pll_en        : 1
[0]emmc_dump_clk_ctl:  up_freq_sel   : 0
[0]emmc_dump_clk_ctl:  freq_sel      : 0x00
[0]emmc_dump_clk_ctl:  tout_cnt      : 0x1
[0]emmc_dump_clk_ctl:  sw_rst_all    : 0
[0]emmc_dump_clk_ctl:  sw_rst_cmd    : 0
